...
/**
 * Architecture: release is park-only; resting tiles must not move on finger-up.
 */
class SettleBoundaryTest {

    private val cell = 100f

    private val mini = PuzzleManifest(
        id = "boundary-mini",
        title = "Boundary mini",
        cols = 2,
        rows = 3,
    ...
    @Test
    fun parkLifted_restingTilesUnchanged() {
        ...
        assertTrue(session.committedAnchor.row > session.startAnchor.row)
        ...
        assertEquals(before, after, "resting tiles must not move during park")
    }

    @Test
    fun settle_alwaysEqualsParkLifted_regardlessOfFinger() {
        val board = PuzzleFixtures.boardFromRowMajor(mini, listOf(5, 4, 3, 2, 1, 0))
        var session = board.beginDrag(LetterSlots.A)!!
